...
Для работы ИИ-консультанта нужны `JWT_SECRET`, `RATE_LIMIT_SECRET`, `OPENROUTER_API_KEY`, `OPENROUTER_MODEL`, `OPENROUTER_FALLBACK_MODEL` и подготовленный `knowledge.db`. По умолчанию генерация идет через `openrouter/free` с fallback на `deepseek/deepseek-v4-flash`. Для отправки лидов нужны `PRAVOVED_REFERRAL_ID` и `PRAVOVED_SECRET`.
