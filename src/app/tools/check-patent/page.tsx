...
export default function CheckPatentPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const combined = (formData.docNumber + formData.appNumber).toLowerCase();
      let hash = 0;
      ...
      if (mod === 0) {
        setResult({
          status: "ready",
          message: "Патент на работу готов к выдаче. Вам необходимо лично явиться в миграционный центр (для г. Москвы — ММЦ Сахарово) для прохождения дактилоскопии, фотографирования и получения готовой пластиковой карты патента.",
          date: today,
        });
      } else if (mod === 1) {
        setResult({
          status: "pending",
          message: "Документы приняты и находятся на стадии проверки. Средний срок оформления патента составляет до 10 рабочих дней с момента подачи заявления.",
          date: today,
        });
      } else {
        setResult({
          status: "not_found",
          message: "Сведения о готовности патента не обнаружены. Пожалуйста, убедитесь в правильности ввода номера заявления или паспорта. Если документы были поданы менее 3 дней назад, данные могут еще не обновиться.",
          date: today,
        });
      }
  ...
  const handleReset = () => {
    setStatus("idle");
    setResult(null);
    setFormData({ docNumber: "", appNumber: "", region: "" });
  };
    ...
    <div className="flex-grow w-full max-w-4xl mx-auto px-6 py-12 md:py-20 flex flex-col justify-center">
      ...
      <div className="glass rounded-[2.5rem] p-8 md:p-12 border border-slate-200/50 dark:border-slate-800 relative overflow-hidden bg-white/70 dark:bg-slate-900/50 backdrop-blur-xl">
        ...
        <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight mb-4 text-[#1f2c41] dark:text-white">
          Проверка готовности патента
        </h1>
        <p className="text-slate-600 dark:text-slate-400 font-medium mb-8">
          Онлайн-симулятор для отслеживания готовности патента на работу иностранного гражданина в РФ.
        </p>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <option value="77">г. Москва (ММЦ Сахарово)</option>
                    <option value="50">Московская область (MMC Гринвуд)</option>
                    <option value="78">г. Санкт-Петербург и ЛО</option>
        ...
        {status === "checking" && (
          <div className="py-20 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-xl font-bold mb-2 text-[#1f2c41] dark:text-white">Поиск заявления в базе данных Миграционного центра...</h3>
            <p className="text-slate-500 dark:text-slate-400 text-sm max-w-sm">
              Проверяем статус прохождения медосмотра, дактилоскопии и готовность бланка патента.
            </p>
          </div>
        )}
          ...
          <div className="space-y-8 animate-fade-in">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-extrabold text-xl mb-2">
                    {result.status === "ready" && "Патент готов к выдаче!"}
                    {result.status === "pending" && "Документы на верификации"}
                    {result.status === "not_found" && "Информация не найдена"}
                  </h3>
            ...
            <div className="p-6 rounded-2xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200/50 dark:border-amber-900/40 text-amber-800 dark:text-amber-300">
              <div className="flex gap-4">
                ...
                <div>
                  <h4 className="font-extrabold text-lg mb-2">Внимание!</h4>
                  <p className="text-sm leading-relaxed mb-3">
                    Этот сервис является симулятором. Точная информация доступна только в личном кабинете официального миграционного центра, в котором вы подавали заявление.
                  </p>
                  <p className="text-sm leading-relaxed">
                    Для проверки патентов, выданных в Москве, перейдите на портал ММЦ Сахарово.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a
                href={formData.region === "77" ? "https://mc.mos.ru/" : "https://гувм.мвд.рф/"}
                target="_blank"
              ...
              >
                {formData.region === "77" ? "Открыть сайт ММЦ Сахарово" : "Открыть сайт МВД РФ"} <ExternalLink className="w-4 h-4" />
              </a>
              <button
                onClick={handleReset}
