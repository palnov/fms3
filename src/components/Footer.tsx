...
export default function Footer({ siteName = "Миграционный справочник", description }: FooterProps) {
    ...
    <footer className={`site-footer border-t border-[#d8dee7] bg-white ${isAdmin ? "" : "public-footer"}`}>
      <div className="site-container grid gap-8 py-10 md:grid-cols-[1fr_auto_auto]">
        <div className="max-w-md">
          <strong className="text-lg font-extrabold tracking-[-0.04em]">{siteName}</strong>
          <p className="site-footer-muted mt-3 text-sm leading-6 text-[#667287]">
            {description || "Понятные инструкции и сервисы на основе законодательства и официальных материалов. Сайт не является государственным органом."}
          </p>
        </div>
